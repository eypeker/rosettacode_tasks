...
use druid::{AppLauncher, PlatformError, Widget, WidgetExt, WindowDesc,TextAlignment};
...
fn get_label(x:usize, y:usize)-> impl Widget<Tg>{
    ...
    let mut l =  Label::new(move |data:&Tg, _env:&_| get_str(data.get_field()[x.clone()][y.clone()]));
    //l.set_text_alignment(druid::TextAlignment::Center);
    l.set_text_size(30.0);
    l.set_text_alignment(TextAlignment::Justified);
    let k = l.fix_height(100.0).fix_width(100.0)
